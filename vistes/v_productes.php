<div class="products-layout">
    ...
    <aside class="filters-sidebar">
        <form action="index.php" method="GET">
            ...
            <div class="filter-group">
                ...
                <label>
                    <input type="radio" name="id_categoria" value="" 
                           <?php echo empty($_GET['id_categoria']) ? 'checked' : ''; ?>> 
                    Totes
                </label>
                <?php if (!empty($llistaCategories)): ?>
                    <?php foreach ($llistaCategories as $cat): ?>
                        <label>
                            <input type="radio" name="id_categoria" 
                                   value="<?php echo $cat['category_id']; ?>"
                                   <?php echo (($_GET['id_categoria'] ?? '') == $cat['category_id']) ? 'checked' : ''; ?>>
            ...
            <div class="filter-group">
                <h3>Preu (€)</h3>
                <input type="number" name="min_preu" class="filter-input" placeholder="Min" min="0"
                       value="<?php echo htmlspecialchars($_GET['min_preu'] ?? ''); ?>">
                <input type="number" name="max_preu" class="filter-input" placeholder="Max" min="0"
                       value="<?php echo htmlspecialchars($_GET['max_preu'] ?? ''); ?>">
            </div>
                ...
                <div class="product-card">
                    <div id="<?php echo $fila['product_id'] ?>" class="product-link">
                        <img class="product-image" 
                             src="./assets/productes/<?php echo $fila['imatge']; ?>" 
                             alt="<?php echo htmlspecialchars($fila['nom']); ?>">
